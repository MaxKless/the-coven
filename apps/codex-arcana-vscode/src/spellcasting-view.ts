...
type Data = {
  ingredients: string[];
  incantations: string[];
  recipes: { name: string; ingredients: string[]; incantations: string[] }[];
};
...
export function initSpellcastingView() {
...
class SpellcastingTreeDataProvider implements TreeDataProvider<TreeItem> {
  ...
  async getSpellcastingData(): Promise<Data> {
    const recipes: Recipe[] = [
      ...
      },
      {
        ...
        description:
          'Creates an extremely strong and durable protective shield.',
      },
      {
        id: 'basic-polymorph',
