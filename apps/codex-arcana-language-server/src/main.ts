...
connection.onCompletion((params) => {
  ...
  const ingredientCompletion = ingredients.map((item) => {
    const completion = CompletionItem.create(item);
    completion.detail = 'Ingredient';
    return completion;
  });

  const incantationCompletion = incantations.map((item) => {
    const completion = CompletionItem.create(item);
  ...
  if (prefixText.includes('addIngredient')) {
    return CompletionList.create(ingredientCompletion);
  }

  if (prefixText.includes('addIncantation')) {
    return CompletionList.create(incantationCompletion);
  }

  return CompletionList.create([
    ...ingredientCompletion,
    ...incantationCompletion,
  ]);
});
